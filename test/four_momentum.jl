...
@testset "$ELTYPE1 $ELTYPE2" for (ELTYPE1, ELTYPE2) in
                                 Iterators.product((Float64, Float32), (Float64, Float32))
    ...
    @testset "coords to dict" begin
        ...
        @test isapprox(
            mom_electron + mom_positron, FourMomentum(2 * E, zeros(ELTYPE1, 3)...)
        )
